(ns podsm.trees.base
  (:require [podsm.trees.parse-model :refer [parse-moa-tree-model-string]]))
...
;;(println (map :name feature-schema))
;;problem comes here
;;(println (map (count (:options feature-schema))))
    (make-tree (map :name feature-schema)
               (map (comp count :options) feature-schema)
               ;;(map :options feature-schema)
               (-> model-string
